...
import 'package:chatsy/elevenLabLib/elevenlabs_config.dart';
...
class ElevenLabsAPI {
  ...
  static void cancelRequests() {
    ...
    ElevenLabsAPI.cancelToken = CancelToken();
  }
  ...
  Future<List<Voice>> listVoices() async {
    try {
      ...
      final List<Map<String, dynamic>> responseList = List.castFrom(response.data["voices"]);
  ...
  Future<bool> editVoiceSettings(String voiceId, double similarityBoost, double stability) async {
    try {
  ...
  Future<Voice> getVoice(String voiceId) async {
    try {
      ...
      final response = await _dio.get('/v1/voices/$voiceId', queryParameters: params);
  ...
  Future<List<int>> getSampleAudio(String voiceId, String sampleId) async {
    try {
      final response = await _dio.get('/v1/voices/$voiceId/samples/$sampleId/audio');
  ...
  Future<File> synthesize(TextToSpeechRequest request, {int optimizeStreamingLatency = 0}) async {
    try {
      ...
      final String fileName = "${localStorage.path}/${request.voiceId}_${DateTime.now().toIso8601String()}.wav";
  ...
  Future<List<HistoryItem>> getHistory({int pageSize = 100}) async {
    try {
      ...
      return (response.data['history'] as List).map((json) => HistoryItem.fromJson(json)).toList();
    } catch (error) {
      throw _handleError(error);
    }
  }
    ...
    if (error is DioException) {
      switch (error.type) {
        ...
        case DioExceptionType.cancel:
          throw RequestCanceledException("cancel by user");
